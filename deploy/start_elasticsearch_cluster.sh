...
function start_master() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -d -p 10200:9200 -p 10300:9300 --restart no --dns $NAMESERVER_IP -h ${MASTER_HOSTNAME}${DOMAINNAME} $VOLUME_MAP $1:$2
    fi
    MASTER=$(sudo docker run -d -p 10200:9200 -p 10300:9300 --restart no --dns $NAMESERVER_IP -h ${MASTER_HOSTNAME}${DOMAINNAME} $VOLUME_MAP $1:$2)
...
function start_workers() {
	...
	hostname="${WORKER_HOSTNAME}${i}${DOMAINNAME}"
        ...
        if [ "$DEBUG" -gt 0 ]; then
	    echo sudo docker run -d -p 1020$i:9200 -p 1030$i:9300 --restart no --dns $NAMESERVER_IP -h $hostname $WORKER_VOLUME_MAP $1:$2
        fi
	WORKER=$(sudo docker run -d -p 1020$i:9200 -p 1030$i:9300 --restart no --dns $NAMESERVER_IP -h $hostname $WORKER_VOLUME_MAP $1:$2)
