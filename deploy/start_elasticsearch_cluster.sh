...
function start_master() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -d -p 9220:9200 -p 9330:9300 --restart no --dns $NAMESERVER_IP -h ${MASTER_HOSTNAME}${DOMAINNAME} $VOLUME_MAP $1:$2
    fi
    MASTER=$(sudo docker run -d -p 9220:9200 -p 9330:9300 --restart no --dns $NAMESERVER_IP -h ${MASTER_HOSTNAME}${DOMAINNAME} $VOLUME_MAP $1:$2)
...
function start_workers() {
	...
	hostname="${WORKER_HOSTNAME}${i}${DOMAINNAME}"
        ...
        if [ "$DEBUG" -gt 0 ]; then
	    echo sudo docker run -d -p 922$i:9200 -p 933$i:9300 --restart no --dns $NAMESERVER_IP -h $hostname $WORKER_VOLUME_MAP $1:$2
        fi
	WORKER=$(sudo docker run -d -p 922$i:9200 -p 933$i:9300 --restart no --dns $NAMESERVER_IP -h $hostname $WORKER_VOLUME_MAP $1:$2)
